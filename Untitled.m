...
Sa1 = align_all_shapes(S, 'init_mean', 1);
Sa2 = align_all_shapes(S, 'init_mean', 5);
Sa3 = align_all_shapes(S, 'init_mean', 10);
...
figure(1);
plot_texture(mask, G(:,5));
figure(2);
plot_texture(mask, Gn(:,5));
...
S = importdata('shortaxes.mat');
Sa = align_all_shapes(S);
pca_shape = compute_pca(Sa, 'reduce', 0.98);
mask = create_mask(pca_shape.mean);
fnames = importdata('samri_data/mri_sa.txt');
G = [];
for i = 1:length(fnames)
...
Gn = photometric_normalization(G);
pca_texture = compute_pca(Gn, 'reduce', 0.98);
% plot_texture(pts, pca_texture.mean);
texture_viewer(pca_texture, mask);
